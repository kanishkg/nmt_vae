...
def get_sentences(im_dict):
    ...
    for key in im_dict.keys():
        ...
        sent_lens_sorted = sorted(sent_lengths)
        min_sent = sent_lengths.index(sent_lens_sorted[0])
        if sent_lens_sorted[1] - sent_lens_sorted[0] <= 1:
            pairs[0] += 1
            orig.append(sents[min_sent])
            altered.append(sents[sent_lengths.index(sent_lens_sorted[1])])
            control_vars.append([0, 0])
        if sent_lens_sorted[4] - sent_lens_sorted[3] <= 1:
            pairs[2] += 1
            orig.append(sents[sent_lengths.index(sent_lens_sorted[4])])
            altered.append(sents[sent_lengths.index(sent_lens_sorted[3])])
            control_vars.append([1, 1])
        if sent_lens_sorted[4] - sent_lens_sorted[0] >= 4:
            pairs[1] += 1
            orig.append(sents[sent_lengths.index(sent_lens_sorted[4])])
            altered.append(sents[sent_lengths.index(sent_lens_sorted[0])])
            control_vars.append([1, 0])

            orig.append(sents[sent_lengths.index(sent_lens_sorted[0])])
            altered.append(sents[sent_lengths.index(sent_lens_sorted[4])])
            control_vars.append([0, 1])
...
def create_datafile(sents, filename, control_vars=[]):
    ...
    for i, sent in enumerate(sents):
        if len(control_vars) > 0:
            sent = sent + '#' + str(
                control_vars[i][0]) + str(control_vars[i][1])
        new_file.write(sent + "\n")
    return len(sents)
...
def process(coco):
    ...
    orig_count = create_datafile(orig_train, "coco_ct_2_tr.src", cv_train)
    altered_count = create_datafile(altered_train, "coco_ct_2_tr.tgt")
    orig_v_count = create_datafile(orig_valid, "coco_ct_2_v.src", cv_valid)
    altered_v_count = create_datafile(altered_valid, "coco_ct_2_v.tgt")
    orig_test_count = create_datafile(orig_test, "coco_ct_2_test.src", cv_test)
    altered_test_count = create_datafile(altered_test, "coco_ct_2_test.tgt")
    
    
    vocab_count = create_vocab_file(
        reverse_dictionary, "coco_ct_2_vocab" + str(vocab_size) + ".src")
