...
def get_iterator(src_dataset,
    ...
    src_tgt_dataset=src_tgt_dataset.filter(
        lambda src, control_vars, tgt: tf.logical_and(tf.size(src) > 0, tf.size(tgt) > 0))

    if src_max_len:
        src_tgt_dataset=src_tgt_dataset.map(
            lambda src, control_vars, tgt: (
                src[:src_max_len], control_vars, tgt),
            num_threads=num_threads,
            output_buffer_size=output_buffer_size)
    if tgt_max_len:
        src_tgt_dataset=src_tgt_dataset.map(
            lambda src, control_vars, tgt: (
                src, control_vars, tgt[:tgt_max_len]),
            num_threads=num_threads,
            output_buffer_size=output_buffer_size)
    if source_reverse:
        src_tgt_dataset=src_tgt_dataset.map(
            lambda src, control_vars, tgt: (tf.reverse(src,
                                                       axis=[0]), control_vars, tgt),
            num_threads=num_threads,
            output_buffer_size=output_buffer_size)
    ...
    src_tgt_dataset=src_tgt_dataset.map(
        lambda src, control_vars, tgt: (src, control_vars,
                          tf.concat(([tgt_sos_id], tgt), 0),
                          tf.concat((tgt, [tgt_eos_id]), 0)),
        num_threads=num_threads, output_buffer_size=output_buffer_size)
    # Add in sequence lengths.
    src_tgt_dataset=src_tgt_dataset.map(
        lambda src, control_vars, tgt_in, tgt_out: (
            src, control_vars, tgt_in, tgt_out, tf.size(src), tf.size(tgt_in)),
        num_threads=num_threads,
        output_buffer_size=output_buffer_size)
    ...
    batched_iter=batched_dataset.make_initializable_iterator()
    (src_ids, control_vars, tgt_input_ids, tgt_output_ids, src_seq_len, tgt_seq_len)=(
