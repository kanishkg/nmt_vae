...
class BaseModel(object):
    ...
    def __init__(self,
        ...
        self.anneal_factor = tf.nn.sigmoid(tf.cast(tf.subtract(
            self.global_step, self.anneal_steps), tf.float64) / self.anneal_scale)
    ...
    def _compute_loss(self, logits, mu, log_sigma_sq):
        ...
        our_dist = tf.contrib.distributions.MultivariateNormalDiag(
            mu, tf.sqrt(tf.exp(log_sigma_sq)))
